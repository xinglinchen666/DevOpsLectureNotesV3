...
Therefore, 
```
rate(request_count{status=" 500"}[1m])/rate(request_count[1m]) OR vector(0)
```

Vector(0) will fill in null with 0
![Alt text](../images/query.png?raw=true)

Try to hit `localhost:5000/test` and `localhost:5000/test1` to generate some data.
